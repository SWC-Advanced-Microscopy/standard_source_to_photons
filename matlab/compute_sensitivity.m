function [result,dataForFit] = compute_sensitivity(frames, count_weight_gamma, min_count_proportion)
    % Compute photon quantal_size
    %
    % function result = compute_sensitivity(frames, count_weight_gamma, min_count_proportion)
    %
    % Purpose
    ...
    %
    %  >> result = compute_sensitivity(frames(:,:,1000:1150));
    ...
    % plotPhotonFit
    %
    % Rob Campbell - SWC 2025 (based on work by Dimitri Yatsenko)
